...
addSettings <- function(result,
  ...
  if (length(settingsColumn) == 0) return(result)

  # add settings
  toJoin <- settingsColumn[settingsColumn %in% colnames(result)]
  result <- result |>
    dplyr::left_join(
      set |> dplyr::select(dplyr::any_of(c("result_id", settingsColumn))),
...
checkSettingsColumns <- function(settingsColumns, result, call = parent.frame()) {
  ...
  if (!is.null(settingsColumns)) {
    ...
    if (length(notPresent) > 0) {
      cli::cli_abort("The following `settings` are not present in settings attribute: {notPresent}.")
    }
  } else {
